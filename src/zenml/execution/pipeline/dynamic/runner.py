...
class DynamicPipelineRunner:
    ...
    def _get_running_upstream_steps_for_startup_controller(
        self,
        inputs: Dict[str, Any],
            ...
            for invocation_id, step_run in monitored_step_runs:
                ...
                if db_status.is_successful:
                    ...
                    if remaining_retries > 0:
                        ...
                        self._startup_controller.notify_isolated_step_finished(
    ...
    def launch_step(
        ...
        if concurrent:
            return self._startup_controller.submit_step(
                step=step,
                ...
                runtime=self._predict_step_runtime(
                    step=step, step_config=step_config
                ),
    ...
    def _prepare_step_launch_for_startup_controller(
        self,
        step: "BaseStep",
        invocation_id: str,
        inputs: Dict[str, Any],
        ...
        """Prepare a ready startup-controlled invocation for dispatch.
    ...
    def _maybe_handle_existing_step_run_for_startup_controller(
        self,
        invocation_id: str,
        prepared: _PreparedStepLaunch,
        wrapped_future: Union["_InlineStepFuture", "_IsolatedStepFuture"],
    ) -> Optional["_StartupStatus"]:
        """Handle reuse of an existing step run in the startup path.

        Args:
            invocation_id: The invocation ID of the step.
            prepared: The prepared launch state.
            wrapped_future: The wrapped startup-managed future.

        Returns:
            The resulting startup-controller status if handled, otherwise `None`.
        """
        step_run = prepared.existing_step_run
        if not step_run:
            return None
        ...
        if step_run.status.is_successful:
            wrapped_future._set_terminal_result(step_run)
            return _StartupStatus.FINISHED_SUCCESS

        if step_run.status.is_failed:
            wrapped_future._set_terminal_exception(
        ...
        if step_run.status == ExecutionStatus.RUNNING:
            ...
            with self._state_lock:
                self._steps_to_monitor[invocation_id] = step_run
            if isinstance(wrapped_future, _IsolatedStepFuture):
                wrapped_future._mark_dispatched()
            return _StartupStatus.ACTIVE_ISOLATED

        return None

    def _launch_scheduled_inline_step(
        self,
        step: "Step",
        remaining_retries: Optional[int] = None,
    ) -> Future["StepRunResponse"]:
        """Launch a scheduled inline step in the runner executor.
    ...
    def _launch_scheduled_isolated_step(
        self,
        step: "Step",
        wrapped_future: Optional["_IsolatedStepFuture"] = None,
    ) -> "StepRunResponse":
        """Launch a scheduled isolated step and start monitoring it.

        Args:
            step: The isolated step to launch.
            wrapped_future: Optional wrapped isolated step future.
    ...
    def _retry_isolated_step(
        ...
        return self._launch_scheduled_isolated_step(
            step=step,
        )
    ...
    def map(
        ...
        return self._startup_controller.submit_map(
            step=step.copy(),
            kwargs=kwargs,
            after=after,
            product=product,
